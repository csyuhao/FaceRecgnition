...
def warp_and_crop_face(src_img,
    ...
    padding = np.array([0, 0, 1], dtype=np.float).reshape(1, 3)
    n_tfm = np.concatenate([tfm, padding], axis=0)

    n_padding = np.array([1] * src_pts.shape[0], dtype=np.float).reshape(src_pts.shape[0], 1)
    n_src_pts = np.concatenate([src_pts, n_padding], axis=1)

    n_dst_pts = np.matmul(n_tfm, n_src_pts.T).T
    n_dst_pts = n_dst_pts / (n_dst_pts[:, -1].reshape(n_dst_pts.shape[0], 1) + 1e-5)

    return face_img, n_dst_pts[:, :-1]
